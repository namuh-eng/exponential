...
if [ -z "${EXPONENTIAL_PROVIDER_CREDENTIAL_ENCRYPTION_KEY:-}" ]; then
  set_env EXPONENTIAL_PROVIDER_CREDENTIAL_ENCRYPTION_KEY "$(random_hex 32)"
fi
if [ -z "${PROVIDER_CREDENTIAL_ENCRYPTION_KEY_ID:-}" ]; then
  set_env PROVIDER_CREDENTIAL_ENCRYPTION_KEY_ID "env:v1"
fi
