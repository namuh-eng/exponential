...
pnpm --filter @exponential/sdk typecheck
pnpm --filter @exponential/sdk test
pnpm --filter @exponential/sdk build

pnpm --filter @exponential/cli typecheck
...
sdk_tarballs=(.release/exponential-sdk-*.tgz)
cli_tarballs=(.release/exponential-cli-*.tgz)
...
if [[ "$dry_run" == "false" ]]; then
  npm view @exponential/sdk version
  npm view @exponential/cli version
fi
