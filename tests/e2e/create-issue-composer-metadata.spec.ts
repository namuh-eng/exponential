...
test.describe("Create issue composer metadata", () => {
  test("selects cycle estimate due date and template metadata and persists it", async ({
    ...
    const title = `Composer metadata ${suffix}`;
    ...
    expect(detailPayload.issue.description).toContain(
      "Template body from composer",
    );
    expect(detailPayload.issue.parentIssueId).toBeTruthy();
  });
});
