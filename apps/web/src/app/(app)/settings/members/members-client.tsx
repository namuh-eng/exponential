...
import type { WorkspaceMembersResponse } from "@/lib/workspace-members";
...
function normalizeMember(
  memberEntry: WorkspaceMembersResponse["members"][number],
): MemberData {
  return {
    ...memberEntry,
    kind: memberEntry.kind as MemberKind,
    role: memberEntry.role as WorkspaceRole,
    status: memberEntry.status as "active" | "pending",
...
export function MembersClient({
  ...
  initialData?: WorkspaceMembersResponse | null;
}) {
  ...
  const [viewerRole, setViewerRole] = useState<WorkspaceRole>(
    (initialData?.viewerRole as WorkspaceRole | undefined) ?? "member",
  );
  const [canInviteMembers, setCanInviteMembers] = useState(
    initialData !== null
      ? (initialData.canInviteMembers ??
          (initialData.viewerRole === "owner" ||
            initialData.viewerRole === "admin"))
      : false,
  );
  const [members, setMembers] = useState<MemberData[]>(
    initialData !== null ? initialData.members.map(normalizeMember) : [],
  );
