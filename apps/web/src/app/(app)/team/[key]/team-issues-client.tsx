...
export function TeamIssuesClient({
  initialData,
}: {
  ...
  const [data, setData] = useState<IssuesResponse | null>(initialData);
  ...
  // The server component seeds initialData for the first paint, so skip the
  // mount fetch when it is present. Subsequent runs (team key changes, sync
  // events) refetch normally to stay live.
  const seededKeyRef = useRef(initialData ? params.key : null);
  useEffect(() => {
    if (seededKeyRef.current === params.key) {
