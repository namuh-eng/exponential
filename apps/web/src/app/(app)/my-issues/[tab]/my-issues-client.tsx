...
export function MyIssuesClient({
  ...
  initialData: MyIssuesResponse | null;
}) {
  ...
  const [data, setData] = useState<MyIssuesResponse | null>(initialData);
  ...
  // The server component seeds initialData for the first paint for the initial
  // tab. Skip the mount fetch when seeded. When activeTab changes (tab
  // navigation), refetch normally to stay live.
  const seededTabRef = useRef(initialData ? activeTab : null);
  useEffect(() => {
    if (seededTabRef.current === activeTab) {
      seededTabRef.current = null;
      return;
    }
    setLoading(true);
    async function fetchMyIssues() {
      try {
        ...
        if (res.ok) {
          const json = (await res.json()) as MyIssuesResponse;
          setData(json);
        }
      } finally {
        setLoading(false);
