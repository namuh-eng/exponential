...
const selectClass =
  "rounded-md border border-[var(--color-border)] bg-[var(--editorial-bg)] px-3 py-2 text-[13px] text-[var(--color-text-primary)]";
...
export function InsightsPanel({
  ...
  if (loading) {
    return (
      <div
        className={`flex h-full items-center justify-center ${shellClass} text-[var(--color-text-secondary)]`}
      >
        Loading analytics...
      </div>
    );
  ...
  return (
    <div
      data-testid="insights-panel-shell"
      className={
        fullScreen
          ? `fixed inset-0 z-50 overflow-auto ${shellClass} p-8`
          : mode === "drawer"
            ? `fixed inset-y-0 right-0 isolate z-40 w-full max-w-[980px] overflow-auto border-l border-[var(--color-border-strong)] ${shellClass} p-6 shadow-2xl`
            : `min-h-full ${shellClass} p-8`
      }
    >
      <div className="mx-auto max-w-[1180px]">
        <div
          className={`mb-6 flex flex-wrap items-start justify-between gap-4 ${panelClass} p-6`}
        >
          <div>
        ...
        {actionMessage && (
          <div
            className={`mb-4 ${panelClass} px-4 py-3 text-[13px] text-[var(--color-text-secondary)]`}
          >
            {actionMessage}
          </div>
        )}
        ...
        <section
          className={`mb-6 grid gap-3 ${panelClass} p-4`}
        ...
        <section className={`mt-6 ${panelClass} p-6`}>
...
function MetricCard({
  ...
  return (
    <button
      className="border border-[var(--color-border-strong)] bg-[var(--color-content-bg)] p-4 text-left transition hover:border-[var(--color-accent)] hover:bg-[var(--color-surface-hover)]"
      onClick={onOpen}
      type="button"
    >
