...
function CategoryHeader({
  ...
  onDefaultChange: (statusId: string) => void;
}) {
  const defaultStatus = statuses.find((status) => status.isDefault);

  return (
    <div className="flex items-center justify-between gap-3 bg-[var(--color-surface)] px-4 py-2">
        ...
        {statuses.length > 0 && (
          <label className="flex items-center gap-2 text-[11px] text-[var(--color-text-tertiary)]">
            ...
            <select
              ...
              className="rounded-md border border-[var(--color-border)] bg-[var(--color-background)] px-2 py-1 text-[11px] text-[var(--color-text-secondary)] outline-none"
            >
              {statuses.map((status) => (
...
export default function TeamIssueStatusesPage() {
  ...
  const teamSettingsBasePath = workspaceSlug
    ? `/${workspaceSlug}/settings/teams/${teamKey}`
  ...
  async function handleDefaultStatusChange(statusId: string) {
    if (!statusId) return;
    await mutate(
      {
        method: "PATCH",
        ...
        {CATEGORY_ORDER.map((category) => (
          <div key={category}>
            <CategoryHeader
              ...
              automationsHref={`${teamSettingsBasePath}/workflows`}
