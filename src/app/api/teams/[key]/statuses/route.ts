...
async function getNextCategoryPosition(
  ...
  const categoryStates = await db
    .select({ id: workflowState.id, position: workflowState.position })
    .from(workflowState)
    .where(
      and(
        eq(workflowState.teamId, teamId),
  ...
  return (
    Math.max(
      -1,
      ...categoryStates
        .filter((state) => state.id !== ignoreId)
        .map((state) => Number(state.position)),
    ) + 1
  );
}
...
export async function POST(
  ...
  const nextPosition = await getNextCategoryPosition(
...
export async function PATCH(
  ...
  const nextPosition = categoryChanged
    ? await getNextCategoryPosition(result.teamRecord.id, nextCategory, id)
    : undefined;
